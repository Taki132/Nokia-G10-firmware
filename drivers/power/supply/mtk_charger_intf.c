...
bool check_wt_chg_sw_term(void)
{
	static struct mtk_charger *pinfo;
	struct power_supply *psy;
//+Bug 635271,lishuwen.wt,2021.03.24,modify, modify get charge status unexpect error case kernel dump
	if (pinfo == NULL) {
...
//	chr_debug("lsw_bat_status %s:%d\n", __func__, pinfo->wt_chg_sw_term);

	return pinfo->wt_chg_sw_term;
}

int check_charger_online(void)
{
	union power_supply_propval prop;
	static struct power_supply *chg_psy;
	int ret;
...
//	pr_notice("lsw_bat_status %s online:%d\n", __func__,prop.intval);

	return prop.intval;
}

int bq2560x_get_charger_status(void)
{
	int ret;
	union power_supply_propval status;
	struct power_supply *chg_psy  = power_supply_get_by_name("bq2560x-gauge");

	if (chg_psy == NULL || IS_ERR(chg_psy)) {
		pr_notice("%s Couldn't get chg_psy\n", __func__);
	} else {
		if(check_wt_chg_sw_term()){
			//pr_notice("lsw_bat_status %s status full:%d\n", __func__, POWER_SUPPLY_STATUS_FULL);
			return POWER_SUPPLY_STATUS_FULL;
		} else {
			ret = power_supply_get_property(chg_psy, POWER_SUPPLY_PROP_STATUS, &status);
			//pr_notice("lsw_bat_status %s status:%d\n", __func__, status.intval);
		}
	}
...
int bq2415x_get_charger_status(void)
{
	int ret;
	union power_supply_propval status;
	struct power_supply *chg_psy  = power_supply_get_by_name("bq2415x-gauge");
...
EXPORT_SYMBOL_GPL(bq2415x_get_charger_status);
#endif
