...
//#if defined(CONFIG_WT_PROJECT_T99653AA1) || defined(CONFIG_WT_PROJECT_T99652AA1)
#if 0
		value.intval=bq2560x_get_charger_status();
//+extb ROG-2543,lishuwen.wt,2020.12.17,modify,MP version no connect usb battery status is DISCHARGING
//1:discharger   ---no connect usb no charging; 2:not charger    ---connect usb but not charging
//frameworks/base/packages/SettingsLib/res/values-zh-rCN/strings.xml  <string name="battery_info_status_discharging">"未在充电"</string> <string name="battery_info_status_not_charging">"已插入电源，但是现在无法充电"</string>
#ifdef WT_COMPILE_FACTORY_VERSION
		if((value.intval == POWER_SUPPLY_STATUS_NOT_CHARGING) && check_charger_online())
#else
		if((value.intval == POWER_SUPPLY_STATUS_NOT_CHARGING) && (check_charger_online()==0))
#endif
//-extb ROG-2543,lishuwen.wt,2020.12.17,modify,MP version no connect usb battery status is DISCHARGING
			value.intval = POWER_SUPPLY_STATUS_DISCHARGING;
//		printk("lsw_bat_status: %d %s\n", value.intval,__func__);
#endif
//-BUG 605310,lishuwen.wt,2020.12.05,modify,status for charger current nod
		ret = sprintf(buf, "%s\n",
