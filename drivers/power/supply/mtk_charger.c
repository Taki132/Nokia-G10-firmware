...
int set_jeita_lcd_on_off(bool onoff)
{
	...
	//chr_debug("lsw_jeita %s: onoff = %d\n", __func__, pinfo->jeita_lcd_on_off);

	return ret;
}

int set_jeita_phone_call(bool onoff)
{
	static struct mtk_charger *pinfo;
	int ret = 0;
	struct power_supply *psy;
	...
	if (1 == onoff)
		pinfo->jeita_phone_call = 1;
	else
		pinfo->jeita_phone_call = 0;

	//chr_debug("lsw_jeita %s: onoff = %d\n", __func__, pinfo->jeita_phone_call);

	return ret;
}

int get_jeita_phone_call(void)
{
	struct power_supply *psy;
	static struct mtk_charger *pinfo;
	int ret;
...
int _mtk_force_stop_charge_func(struct mtk_charger *info, bool en)
{
	...
	if(en) {
		info->force_stop_charge = true;	
	} else {
		info->force_stop_charge = false;
	}
...
//+Bug 604975,lishuwen.wt,2020.12.09,jeita for current limit
	...
	if (sw_jeita->sm != TEMP_T2_TO_T3) {
		if (sw_jeita->sm == TEMP_ABOVE_T4) {
			...
			sw_jeita->cv = info->data.jeita_temp_t3_to_t4_cv;
			if(info->jeita_lcd_on_off)
				sw_jeita->cc = 1200000;
			else
				sw_jeita->cc = info->data.jeita_temp_t3_to_t4_cc;

			if(info->jeita_phone_call)
				sw_jeita->cc = 850000;
		...
		sw_jeita->cv = 0;
		//if(info->jeita_lcd_on_off){
		//	if((42 <= info->battery_temp) && (info->battery_temp < 45))
		//		sw_jeita->cc = 1200000;
		//	else if ((40 <= info->battery_temp) && (info->battery_temp < 42))
		//		sw_jeita->cc = 1500000;
		//	else
	...
	//chr_err("lsw_jeita [SW_JEITA]preState:%d newState:%d tmp:%d cv:%d cc:%d jeita_lcd_on_off:%d jeita_phone_call:%d\n",
		//sw_jeita->pre_sm, sw_jeita->sm, info->battery_temp,
		//sw_jeita->cv, sw_jeita->cc, info->jeita_lcd_on_off,info->jeita_phone_call);
//-Bug 604975,lishuwen.wt,2020.12.09,jeita for current limit
//+chk 69775,lishuwen.wt,2020.12.02,jeita for current limit
#elif defined(CONFIG_WT_PROJECT_T99652AA1) || defined(CONFIG_WT_PROJECT_T99653AA1)
	if (sw_jeita->sm != TEMP_T2_TO_T3) {
		if (sw_jeita->sm == TEMP_ABOVE_T4) {
			sw_jeita->cv = info->data.jeita_temp_above_t4_cv;
			sw_jeita->cc = info->data.jeita_temp_above_t4_cc;
			...
			if(info->jeita_phone_call)
//+bug 584789, liyiying.wt, mod, 2021/01/09, modify jeita phone call current 45~55
				sw_jeita->cc = 700000;
//-bug 584789, liyiying.wt, mod, 2021/01/09, modify jeita phone call current 45~55
			...
			sw_jeita->cv = info->data.jeita_temp_t1_to_t2_cv;
			sw_jeita->cc = info->data.jeita_temp_t1_to_t2_cc;
//+bug 584789, liyiying.wt, add, 2021/01/09, add jeita phone call current 0~15
			if(info->jeita_phone_call)
				sw_jeita->cc = 700000;
//-bug 584789, liyiying.wt, add, 2021/01/09, add jeita phone call current 0~15
		} else if (sw_jeita->sm == TEMP_T0_TO_T1) {
			...
			if(info->jeita_phone_call)
//+bug 584789, liyiying.wt, mod, 2021/01/05, add jeita phone call current
				sw_jeita->cc = 700000;
//-bug 584789, liyiying.wt, mod, 2021/01/05, add jeita phone call current
		}
	} else {
		...
		if(info->jeita_phone_call)
//+bug 584789, liyiying.wt, mod, 2021/01/09, modify jeita phone call current
			sw_jeita->cc = 700000;
//-bug 584789, liyiying.wt, mod, 2021/01/09, modify jeita phone call current
	}
	//chr_err("lsw_jeita [SW_JEITA]preState:%d newState:%d tmp:%d cv:%d cc:%d jeita_lcd_on_off:%d jeita_phone_call:%d\n",
...
//+bug589756, yaocankun, 20201030, add, fix chg term current issue
#if defined(CONFIG_WT_PROJECT_T99653AA1) || defined(CONFIG_WT_PROJECT_T99652AA1)  //bug 584789, liyiying.wt, add, 2020/11/06, add T99652 support
//static int wt_chg_sw_term = 0;
#include <linux/power_supply.h>
#include "mtk_battery.h"
#define WT_CHG_TERM_CURRENT (250)  //bug 631190, liyiying.wt, 2020/03/04, mod, fix cut-off current too high
...
static int mtk_chg_check_term(struct mtk_charger *info)
{
	...
	power_supply_get_property(bat_psy, POWER_SUPPLY_PROP_CURRENT_NOW,
		&propval);
//+bug 631190, liyiying.wt, 2020/03/04, mod, fix cut-off current too high
...
//-bug 631190, liyiying.wt, 2020/03/04, mod, fix cut-off current too high
			...
			&& (info->wt_chg_sw_term == 0))
	{
		if((bat_vol > (chg_cv - WT_CHG_CV_HYS))			//vbat > 4385
			&& (ui_soc == 100)
			&& (bat_cur < WT_CHG_TERM_CURRENT))  //bug 631190, liyiying.wt, 2020/03/04, mod, fix cut-off current too high
		{
			if(check_count > 3)
			{
				info->wt_chg_sw_term = 1;
...
//			bs_data->bat_status = POWER_SUPPLY_STATUS_CHARGING;
	...
	pr_err("cv:%d, soc:%d, vbat:%d, ibat:%d, bat status:%d, term flag:%d, count:%d\n",
		chg_cv, ui_soc, bat_vol, bat_cur, bat_sta, info->wt_chg_sw_term,check_count);
	return ret;
...
//Modify by xu_shengjie@hoperun.com for WVR-12711 setting maximum charge percentage 2022-05-16 begin
#if defined(CONFIG_WT_PROJECT_T99653AA1) || defined(CONFIG_WT_PROJECT_T99652AA1) || defined(CONFIG_WT_PROJECT_T99651AA2)
	if(info->force_stop_charge == true){
		charging = false;
		goto stop_charging;
	}
#endif
...
//+chk SC14476,lishuwen.wt,2021.01.13,modify,ATO add charger current control in kernel
...
#ifdef WT_COMPILE_FACTORY_VERSION
#if defined(CONFIG_WT_PROJECT_T99653AA1) || defined(CONFIG_WT_PROJECT_T99652AA1)
#define ATO_HIGH_CAP 78
#define ATO_LOW_CAP 62
static void set_ato_charging_status(struct mtk_charger *info)
{
	int ui_soc;
	ui_soc = get_uisoc(info);
	if(ui_soc >= ATO_HIGH_CAP){
		info->disable_charging_test = 1;
		printk("lsw_ato_charging %s ui_soc %d >= %d disable charging\n",__func__,ui_soc,ATO_HIGH_CAP);
	}else if(ui_soc <= ATO_LOW_CAP){
		info->disable_charging_test = 0;
		printk("lsw_ato_charging %s ui_soc %d <= %d enable charging\n",__func__,ui_soc,ATO_LOW_CAP);
	}
