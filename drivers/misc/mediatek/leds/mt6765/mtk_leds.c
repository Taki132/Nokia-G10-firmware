...
#if defined(CONFIG_WT_PROJECT_T99621AA3) || defined(CONFIG_WT_PROJECT_T99651AA2) || defined(CONFIG_WT_PROJECT_T99653AA1) || defined(CONFIG_WT_PROJECT_T99652AA1)

#ifdef CONFIG_MTK_AAL_SUPPORT

#define BOOST_MODE_LEVEL 205
#define MAX_BACKLIGHT_LEVEL 255
u32 g_last_bl = 205;

static int led_trigger_dim(struct mt65xx_led_data *led_data,int from, int to, int div)

{
	int temp,temp1,lvl;
	unsigned t=2;
	
	pr_info("[Display] led_trigger_from:from = %d, to = %d, div = %d\n",from,to,div);

	temp1 = (to-from) / div;
	for (temp=1; temp<=div; temp++)
	{
		lvl = from + ( temp1 * temp);
		pr_debug("[Display]<===>lvl[%d]=%d,\n",temp,lvl);

		backlight_debug_log(led_data->level, lvl);
		...
		disp_aal_notify_backlight_changed((((1 <<
				MT_LED_INTERNAL_LEVEL_BIT_CNT)
						    ...
						    127) / 255);
		led_data->level=lvl;
		//t =abs(temp1*20);//RNN-4642, liuchunyang.wt, MODIFY, 20210322, modify lcd boost aboot delay
		pr_info("[Display]t==%d\n",t);
		mdelay(2);
...
void mt_mt65xx_led_set(struct led_classdev *led_cdev, enum led_brightness level)
{
	...
	if (led_data->level != level) {
		if (strcmp(led_data->cust.name, "lcd-backlight") != 0) {
			...
			//if ((led_data->level==MAX_BACKLIGHT_LEVEL) || (level == MAX_BACKLIGHT_LEVEL)){//RNN-4642, liuchunyang.wt, MODIFY, 20210322, modify lcd boost aboot 255 turn off
				if(level == MAX_BACKLIGHT_LEVEL) {
				pr_err("[Display]come in boost mode level=%d,led_data->level=%d\n",level,led_data->level);
				diff= level - led_data->level;
				t=abs(diff);
				if(t<=10){
					led_div = t;
					pr_debug("[Display]<1====>div=%d \n",led_div);
				}else{
					led_div = t /10;
					pr_debug("[Display]<2====>div=%d \n",led_div);

				}

				led_trigger_dim(led_data , led_data->level, level,led_div);
				goto end ;
			}else{
				//pr_err("[Display] come in normal mode\n");
				led_data->level = level;
				if ((!strncmp(Lcm_name,"ft8615_hdp_vdo_incell_lide",sizeof(Lcm_name)))&&(level==2)){
				pr_debug("[Display]lide panel bl set 1\n");
					level = 1;
				}
