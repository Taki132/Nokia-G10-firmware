...
//MODIFY,20211206, make L-sensor report lux stably every cycle
#if (defined CONFIG_WT_PROJECT_T99652AA1) || (defined CONFIG_WT_PROJECT_T99653AA1) || (defined CONFIG_WT_PROJECT_T99651AA2)
bool als_report_always = true;
#else
bool als_report_always = false;
#endif
...
int als_data_report_t(int value, int status, int64_t time_stamp)
{
	...
	if ((value != last_als_report_data) || als_report_always) {
		event.handle = ID_LIGHT;
		event.flush_action = DATA_ACTION;
		event.word[0] = value;
		event.status = status;
