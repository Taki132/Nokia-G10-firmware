...
#ifdef CONFIG_TOUCHSCREEN_HIMAX_CHIPSET_651R
//+Chk37354, sheqihao.wt, MODIFY, 20201127, fix tp abnormal after esd test 
/********************************************************************************
*FUNCTION

*  disp_ts_esd_suspend_cmd

*DESCRIPTION

*   Himax tp Recovery function
...
* BOOL VALUE
********************************************************************************/
int disp_ts_esd_suspend_cmd(struct disp_lcm_handle *plcm)
{
	struct LCM_DRIVER *lcm_drv = NULL;
	...
	if (_is_lcm_inited(plcm)) {
		lcm_drv = plcm->drv;
		if (lcm_drv->esd_suspend_cmd) {
			lcm_drv->esd_suspend_cmd();
		} else {
			DISPERR("WARN ERROR, lcm_drv->esd_suspend_cmd is null\n");
			return -1;
		}
