...
WT_PROINFO_CREATE_ATTR(jio_simlock)
...
WT_PROINFO_CREATE_ATTR(off_mode_charge)
...
static int wt_create_device_files(void)
{
    ...
    if (rc)
        return rc;
    rc = device_create_file(proinfo_root_dev, &dev_attr_jio_simlock);
    if (rc)
        return rc;
    ...
    rc = device_create_file(proinfo_root_dev, &dev_attr_off_mode_charge);
    if (rc)
        return rc;
    ...
    */
    ...
    switch(type) {
        ...
        case WT_PROINFO_factoryreset_date:
            ...
            break;
        case WT_PROINFO_jio_simlock:
            printk("%s read jio simlock[%d]\n", __func__, type);
            ret = emmc_partition_rw("oem",  0, type * SIZE_4K,(char *) fbuf, (unsigned long) WT_PROINFO_STRING_LEN);
            printk("%s read jio simlock[%d]:%s\n", __func__, type, fbuf);
            break;
        case WT_PROINFO_off_mode_charge:
            printk("%s read off_mode_charge \n", __func__);
...
static int wt_proinfo_write(wt_proinfo_type type, const char* buf, int len)
{
    ...
    if((type != WT_PROINFO_block_fastboot_mode) && (type != WT_PROINFO_block_factory_reset)
        && (type != WT_PROINFO_factoryreset_date) && (type != WT_PROINFO_jio_simlock)
        && (type != WT_PROINFO_remote_lock_value)
        && (type != WT_PROINFO_sku_id) && (type != WT_PROINFO_off_mode_charge)) {
