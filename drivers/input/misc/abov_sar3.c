...
//Bug616828 tangsha.wt, MOD,20210115, recalibrate sar when USB is plugged in
...
//+chk37581 tangsha.wt, MOD,20201123, add sar sensor firmware to finish update
        manual_offset_calibration(this);
        msleep(500);
//-chk37581 tangsha.wt, MOD,20201123, add sar sensor firmware to finish update
  //+Bug 436909 huangcunzhu_wt,20190416,MODIFY sar work when sleeping
...
//+Extb RNN-4012 hehao.wt, MOD,20210129, fix sar not report the first value when USB is plugged in
 if(this->irq_disabled == 1){
   LOG_DBG("Usb insert,cap sensor disabled.\n");
   return;
 }
//-Extb RNN-4012 hehao.wt, MOD,20210129, fix sar not report the first value when USB is plugged in
 input_report_abs(input_top, ABS_DISTANCE, 0);
...
static int abov_fw_update(bool force)
{
 ...
 LOG_INFO("%s: call in %d, ic ver %d mono %d (file ver %d mono %d)\n", __func__, force, fw_version, fw_modelno, fw_file_version,fw_file_modeno);
...
#if WT_ADD_SAR_HARDWARE_INFO
   read_register(this, ABOV_VERSION_REG, &fw_version);
//Bug589755 tangsha.wt, MOD,20201027, modify SAR sensor hardware info
