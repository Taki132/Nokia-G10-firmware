...
//+ add by wanglongfei.wt for  tp drivers  20190830
enum module_name{
	TXD_HX83102_D_MODULE = 0,
	TRULY_F8006P_MODULE = 1,
	LIDE_FT8615_MODULE = 2,
	TXD_FT8006P_MODULE = 3,
	LIDE_HX83102D_MODULE = 4,
	TXD_ILITEK_MODULE = 5,
	LC_HX83102D_MODULE = 6,//bug591035, sheqihao.wt, MODIFY, 20200929, for Display Bringup
	SKYWORTH_ILITEK_MODULE = 7,//bug589756, sheqihao.wt, MODIFY, 20201009, TP ILI9882H Bringup.
	HX_ILITEK_MODULE = 8,//Mtr4421, sheqihao.wt, ADD, 20201013, for T99651AA2 tp ili9882h_hx Bringup
	LIDE_ICNL9911C_MODULE = 9,//bug584789, liuchunyang.wt, MODIFY, 20201111, TP incl9911 Bringup
};
//- add by wanglongfei.wt for  tp drivers  20190830
			...
			unsigned long event, void *data)
{
	...
	if ((event == FB_EVENT_BLANK) || (event == FB_EARLY_EVENT_BLANK)) {
	    ...
	    TPD_DMESG("fb_notify blank=%d, event=%ld\n", blank, event);

        if(event == FB_EVENT_BLANK) {
		    if(blank == FB_BLANK_UNBLANK) {
			    TPD_DMESG("LCD ON Notify, tpd_suspend_flag = %d\n", tpd_suspend_flag); 
...
static int __parse_tag_videolfb(struct device_node *node)
{
	...
	if(0==strcmp(mtkfb_lcm_name,"hx83102d_hdp_vdo_incell_txd")){
		lcm_name = TXD_HX83102_D_MODULE;
		strcpy((char *)tp_name, "hxmax_generic");
	}else if(0==strcmp(mtkfb_lcm_name,"ft8006p_hdp_vdo_incell_truly")){
		lcm_name = TRULY_F8006P_MODULE;
		strcpy((char *)tp_name, "fts_ts_ft8006p");
	}else if(0==strcmp(mtkfb_lcm_name,"ft8615_hdp_vdo_incell_lide")){
		lcm_name = LIDE_FT8615_MODULE;
		strcpy((char *)tp_name, "fts_ts_ft8615");
	}else if(0==strcmp(mtkfb_lcm_name,"ft8006p_hdp_vdo_incell_txd_boe")){
		lcm_name = TXD_FT8006P_MODULE;
		strcpy((char *)tp_name, "fts_ts_ft8006p_boe");
	}else if(0==strcmp(mtkfb_lcm_name,"hx83102d_hdp_vdo_incell_lide")){
		lcm_name = LIDE_HX83102D_MODULE;
		strcpy((char *)tp_name, "hxmax_generic");
	}else if(0==strcmp(mtkfb_lcm_name,"ili9881h_hd_plus_vdo_txd")){
		lcm_name = TXD_ILITEK_MODULE;
		strcpy((char *)tp_name, "ILITEK_TDDI");
	//+bug591035, sheqihao.wt, MODIFY, 20200929, for Touch Bringup
	}else if(0==strcmp(mtkfb_lcm_name,"hx83102d_hdp_vdo_incell_lc")){
		lcm_name = LC_HX83102D_MODULE;
		strcpy((char *)tp_name, "hxmax_generic");
	}
	//-bug591035, sheqihao.wt, MODIFY, 20200929, for Touch Bringup
	//+bug589756, liuchunyang.wt, MODIFY, 20201126, compatible EVT panel and DVT later panel for ili9882
	else if(0==strcmp(mtkfb_lcm_name,"ili9882h_hdp_vdo_incell_sky_evt")){ 
		lcm_name = SKYWORTH_ILITEK_MODULE;  
		strcpy((char *)tp_name, "ILITEK_TDDI_9882H");
	}
	//-bug589756, liuchunyang.wt, MODIFY, 20201126, compatible EVT panel and DVT later panel for ili9882
	//+bug589756, sheqihao.wt, MODIFY, 20201009, TP ILI9882H Bringup.
	else if(0==strcmp(mtkfb_lcm_name,"ili9882h_hdp_vdo_incell_skyworth")){ 
		lcm_name = SKYWORTH_ILITEK_MODULE;  
		strcpy((char *)tp_name, "ILITEK_TDDI_9882H");
	}
	//-bug589756, sheqihao.wt, MODIFY, 20201009, TP ILI9882H Bringup.
	//+Mtr4421, sheqihao.wt, ADD, 20201013, for T99651AA2 tp ili9882h_hx Bringup.
	else if(0==strcmp(mtkfb_lcm_name,"ili9882h_hdp_vdo_incell_hx")){ 
		lcm_name = HX_ILITEK_MODULE;  
		strcpy((char *)tp_name, "ILITEK_TDDI_9882H");
	}	
	//-Mtr4421, sheqihao.wt, ADD, 20201013, for T99651AA2 tp ili9882h_hx Bringup

	//+bug584789, liuchunyang.wt, MODIFY, 20201111, TP incl9911 Bringup
	else if(0==strcmp(mtkfb_lcm_name,"icnl9911_hdp_vdo_incell_lide")){ 
		lcm_name = LIDE_ICNL9911C_MODULE;  
		strcpy((char *)tp_name, "chipone-tddi");
	}	
	//-bug584789, liuchunyang.wt, MODIFY, 20201111, TP incl9911 Bringup

//+Chk67730, sheqihao.wt, ADD, 20201019, for T99651AA2 lcd virtual panel add.
#ifdef CONFIG_WT_PROJECT_T99651AA2
	else if(0==strcmp(mtkfb_lcm_name,"Simulator_dsi_vdo_default_NoPanel")){
		printk("[MTK_TPD] mtkfb_lcm_name is %s\n",mtkfb_lcm_name);
		lcm_name = -1;
	}
#endif
//-Chk67730, sheqihao.wt, ADD, 20201019, for T99651AA2 lcd virtual panel add.
...
static int tpd_probe(struct platform_device *pdev)
{
	...
	int ret = 0;
	   ...
	   _parse_tag_videolfb();// add by wanglongfei.wt for  tp drivers 20190830

//+Chk67730, sheqihao.wt, ADD, 20201019, for T99651AA2 lcd virtual panel add.
#ifdef CONFIG_WT_PROJECT_T99651AA2
	if(lcm_name == -1){
       printk("[MTK_TPD] mtkfb_lcm_name is %s, do not loading touch drivers\n",mtkfb_lcm_name);
       return 0;
       }
#endif
//-Chk67730, sheqihao.wt, ADD, 20201019, for T99651AA2 lcd virtual panel add.
...
/*for some projects, we do not use mtk framebuffer*/
	...
	if (2560 == TPD_RES_X)
		TPD_RES_X = 2048;
	if (1600 == TPD_RES_Y)
		TPD_RES_Y = 1600;//bug589756, sheqihao.wt, MODIFY, 20201009, TP ILI9882H Bringup.
	...
	for (i = 1; i < TP_DRV_MAX_COUNT; i++) {
		/* add tpd driver into list */
		if (tpd_driver_list[i].tpd_device_name != NULL) {
			//+ add by wanglongfei.wt for  ILITEK tp compitible focal tp	normal 20190830
