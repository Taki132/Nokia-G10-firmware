    ...
    struct cts_upgrade_fw_ioctl_data *ioctl_data)
{
    ...
    if (!!(ioctl_data->flags & CTS_IOCTL_UPGRADE_FW_FLAG_FILE)) {
        ...
        if (IS_ERR(filepath)) {
            ///int ret = PTR_ERR(filepath); //bug624366, sheqihao.wt, add, 20210204,  CID164562 fix Function "cts_tool_ioctl"  overflow
            cts_err("Memdump filepath to kernel failed");
    ...
    if (!!(ioctl_data->flags & CTS_IOCTL_UPGRADE_FW_FLAG_FW_DATA)) {
        ...
        if (IS_ERR(firmware.data)) {
           // int ret = PTR_ERR(firmware.data);//bug624366, sheqihao.wt, add, 20210204,  CID164562 fix Function "cts_tool_ioctl"  overflow
            cts_err("Memdump fw data to kernel failed");
            return PTR_ERR(firmware.data);
        }
        ...
        unsigned long arg)
{
    ...
    case CTS_TOOL_IOCTL_TEST:{
        ...
        if (IS_ERR(tests_pa)) {
            //int ret = PTR_ERR(tests_pa);//bug624366, sheqihao.wt, add, 20210204,  CID164562 fix Function "cts_tool_ioctl"  overflow
            cts_err("Memdump test param to kernel failed");
            return PTR_ERR(tests_pa);
        }
    ...
    case CTS_TOOL_IOCTL_RDWR_REG:{
        ...
        if (IS_ERR(regs_pa)) {
            //int ret = PTR_ERR(regs_pa);//bug624366, sheqihao.wt, add, 20210204,  CID164562 fix Function "cts_tool_ioctl"  overflow
            cts_err("Memdump cts_rdwr_reg to kernel failed");
            return PTR_ERR(regs_pa);
        }
