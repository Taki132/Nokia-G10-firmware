...
//bug598911, liujin.wt, add, 20210316, for audio record pop
//static unsigned int pmic_default_drv = 0;
	...
	if ((bEnable == true) && (mAnaSuspend == false)) {
		...
		if (mCodec_data->mAudio_Ana_DevicePower
			...
			Speaker_Amp_Change(false);
                ...
                //add 20211123 for add voice hac feature
                if (mCodec_data->mAudio_Ana_DevicePower
                       [AUDIO_ANALOG_DEVICE_OUT_HACSET] == true) {
                       mCodec_data->mAudio_Ana_DevicePower
                               [AUDIO_ANALOG_DEVICE_OUT_HACSET] = false;
                       Voice_Hac_Change(false);
                }
                //add 20211123 for add voice hac feature
		/*Ana_Log_Print();*/
		mAnaSuspend = true;
...
static void set_playback_gpio(bool enable)
{
	if (enable) {
		...
		Ana_Set_Reg(DRV_CON2, 0xcccc, 0xffff);*/
		//-bug598911, liujin.wt, add, 20210316, for audio record pop
	} else {
		...
		//+bug598911, liujin.wt, add, 20210316, for audio record pop
		/*Ana_Set_Reg(DRV_CON3, pmic_default_drv,
					0xffff);
		Ana_Set_Reg(DRV_CON2, pmic_default_drv,
					0xffff);*/
		//-bug598911, liujin.wt, add, 20210316, for audio record pop
	}
}
static void set_capture_gpio(bool enable)
{
	if (enable) {
		...
		//+bug598911, liujin.wt, add, 20210316, for audio record pop
		Ana_Set_Reg(SMT_CON1, 0x0ff0, 0x0ff0);
		Ana_Set_Reg(DRV_CON3, 0xaaaa, 0xffff);
		//Ana_Set_Reg(DRV_CON2, 0xcccc, 0xffff);
		//-bug598911, liujin.wt, add, 20210316, for audio record pop
	} else {
...
//add 20211123 for add voice hac feature
static int Voice_Hac_Get(struct snd_kcontrol *kcontrol,
                        struct snd_ctl_elem_value *ucontrol)
{
    pr_debug("Voice_Hac_Get = %d\n",
        mCodec_data->mAudio_Ana_DevicePower
        [AUDIO_ANALOG_DEVICE_OUT_HACSET]);
    ucontrol->value.integer.value[0] =
        mCodec_data->mAudio_Ana_DevicePower
            [AUDIO_ANALOG_DEVICE_OUT_HACSET];
    return 0;
...
static int Voice_Hac_Set(struct snd_kcontrol *kcontrol,
                        struct snd_ctl_elem_value *ucontrol)
{
    mutex_lock(&Ana_Power_Mutex);
    pr_debug("%s() value = %ld\n ", __func__,
...
static const struct snd_kcontrol_new mt6357_snd_controls[] = {
	...
	SOC_ENUM_EXT("Apply_N12DB_Gain", Audio_DL_Enum[14],
        ...
        SOC_ENUM_EXT("Voice_Hac_Switch", Audio_DL_Enum[15],
                     Voice_Hac_Get,Voice_Hac_Set),
};//add 20211123 for add voice hac feature
...
static void mt6357_codec_init_reg(struct snd_soc_component *component)
{
	...
	//bug598911, liujin.wt, add, 20210316, for audio record pop
	/* get pmic driving */
	//pmic_default_drv = Ana_Get_Reg(DRV_CON3);
	/* set gpio */
	set_playback_gpio(false);
	set_capture_gpio(false);
	audckbufEnable(false);
