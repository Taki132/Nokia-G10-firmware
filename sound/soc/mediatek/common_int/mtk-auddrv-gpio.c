...
//20211123 add, audio bringup
#ifdef CONFIG_AUDIOPA_AW87318
...
#endif
//20211123 add, audio bringup
static int hacpa_gpio;//IRM-17045 xiangyujie.wt add 20200914 for add voice hac feature
static DEFINE_MUTEX(gpio_request_mutex);

//20211123 add, for awinic pa timing sequence less than 10us
	...
	if (np) {
		...
		if (ret) {
			pr_err("%s : gpio_request failed\n", __FILE__);
                ...
                ret = of_get_named_gpio(np, "hacpa-gpio", 0);
                if (ret < 0)
                        pr_err("%s: get hacpa-gpio failed (%d)", __FILE__, ret);
                else
                        hacpa_gpio = ret;
                pr_err("%s hacpa_gpio num =%d\n", __func__, hacpa_gpio);

                ret = gpio_request(hacpa_gpio,"hacpa-gpio" );
                if (ret) {
                        pr_err("%s : gpio_request failed\n", __FILE__);
                }
...
#define AW87318_PA_MODE 3
#endif
#ifdef CONFIG_WT_PROJECT_T99651AA2
#define AW87318_PA_MODE 1
#endif
#ifdef CONFIG_WT_PROJECT_T99653AA1
#define AW87318_PA_MODE 1
#endif
#ifdef CONFIG_AUDIOPA_AW87318
void Awin_Speaker_Amp_Change(bool enable)
{
	int i;
	...
	if(enable) {
		msleep(1);
		preempt_disable();
		for(i=0; i<AW87318_PA_MODE; i++) {
			ndelay(1000);
			//gpio_direction_output(extamp_gpio, 0);
			gpio_set_value(extamp_gpio, 0);
			//ret=gpio_get_value(extamp_gpio);
			//pr_err("cjdon1=%s@%d", __func__, ret);
			ndelay(1000);
			//gpio_direction_output(extamp_gpio, 1); // more than 10us
			gpio_set_value(extamp_gpio, 1);  // less than 10us
			//ret=gpio_get_value(extamp_gpio);
			//pr_err("cjdon=%s@%d", __func__, ret);
		}
		preempt_enable();
		usleep_range(13 * 1000, 15 * 1000);
	}
	else {
		gpio_direction_output(extamp_gpio, 0);
		pr_err("cjdoff=%s@%d", __func__, __LINE__);
	}
}
#endif
//20211123 add, for awinic pa timing sequence less than 10us
void Voice_Hac_Change(bool enable)
{
    int ret = 0;
    pr_err("%s %d",  __func__, __LINE__);
    if(enable){
        msleep(1);
        preempt_disable();
        gpio_set_value(hacpa_gpio, 0);
        ret = gpio_get_value(hacpa_gpio);
        ndelay(1000);
        gpio_set_value(hacpa_gpio,1);
        ret = gpio_get_value(hacpa_gpio);
        preempt_enable();
    }
    else{
        gpio_direction_output(hacpa_gpio,0);
        pr_err("%s@%d\n", __func__, __LINE__);
    }
}
//-IRM-17045 xiangyujie.wt add 20200914 for add voice hac feature
